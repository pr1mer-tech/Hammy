...
import { UNISWAP_V2_FACTORY_ABI, UNISWAP_V2_FACTORY, ERC20_ABI, UNISWAP_V2_PAIR_ABI } from "@/lib/constants"
...
export function PositionsTable() {
  ...
  const { data: pairData } = useReadContracts({
    contracts: commonPairs.map((pair) => ({
      ...
      args: [
        pair.tokenA.address === zeroAddress ? "0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2" : pair.tokenA.address,
        pair.tokenB.address === zeroAddress ? "0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2" : pair.tokenB.address,
      ],
    })),
    query: {
  ...
  const { data: balanceData } = useReadContracts({
    ...
    query: {
      enabled: isConnected && !!address && !!pairData && pairData.length > 0,
    },
  })
  ...
  useEffect(() => {
    const fetchPositions = async () => {
      if (!isConnected || !address || !pairData || !balanceData || !reservesData) {
        setIsLoading(false)
        return
      }
  ...
  return (
    <Card className="card-gradient rounded-2xl border-0 overflow-hidden h-full">
      <CardHeader className="pb-2">
        <CardTitle className="text-xl text-blue-800">Your Liquidity Positions</CardTitle>
      </CardHeader>
      <CardContent>
        {isLoading ? (
          <div className="text-center py-8 text-blue-600">Loading your positions...</div>
        ) : !isConnected ? (
            ...
            <table className="w-full positions-table">
              <thead>
                <tr className="bg-blue-50">
                  <th className="text-left py-3 px-4 text-blue-700 font-medium">Pool</th>
                  <th className="text-right py-3 px-4 text-blue-700 font-medium">My Liquidity</th>
                  <th className="text-right py-3 px-4 text-blue-700 font-medium">Value</th>
                </tr>
              </thead>
              <tbody>
                {positions.map((position) => (
                  <tr key={position.pairAddress} className="hover:bg-blue-50/50">
                    <td className="py-4 px-4">
                      <div className="flex items-center gap-2">
                        <div className="flex -space-x-2">
                          {getTokenIconUrl(position.tokenA.address) ? (
                            <Image
                              src={getTokenIconUrl(position.tokenA.address) || "/placeholder.svg"}
                          ...
                          {getTokenIconUrl(position.tokenB.address) ? (
                            <Image
                              src={getTokenIconUrl(position.tokenB.address) || "/placeholder.svg"}
                    ...
                    <td className="py-4 px-4 text-right">
                      <div className="text-blue-800 font-medium">
                        {position.tokenA.balance} {position.tokenA.symbol}
                      </div>
                      <div className="text-blue-800 font-medium">
                        {position.tokenB.balance} {position.tokenB.symbol}
                      </div>
                      <div className="text-xs text-blue-600">
                        {Number.parseFloat(position.lpTokens).toFixed(6)} LP Tokens
                      </div>
                    </td>
                    <td className="py-4 px-4 text-right font-medium text-blue-800">{position.value}</td>
                  </tr>
                ))}
              </tbody>
            </table>
