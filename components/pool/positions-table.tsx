...
export function PositionsTable() {
  ...
  useEffect(() => {
    ...
    fetchPositions()
  }, [isConnected, address, pairData, balanceData, reservesData, tokens])

  return (
    <Card className="card-gradient rounded-2xl border-0 overflow-hidden h-full">
      <CardHeader className="pb-2">
        <CardTitle className="text-xl text-amber-800">Your Liquidity Positions</CardTitle>
      </CardHeader>
      <CardContent>
        {isLoading ? (
          <div className="text-center py-8 text-amber-600">Loading your positions...</div>
        ) : !isConnected ? (
            ...
            <table className="w-full positions-table">
              <thead>
                <tr className="bg-amber-50">
                  <th className="text-left py-3 px-4 text-amber-700 font-medium">Pool</th>
                  <th className="text-right py-3 px-4 text-amber-700 font-medium">My Liquidity</th>
                  <th className="text-right py-3 px-4 text-amber-700 font-medium">Value</th>
                </tr>
              </thead>
              <tbody>
                {positions.map((position) => (
                  <tr key={position.pairAddress} className="hover:bg-amber-50/50">
                    ...
                    <td className="py-4 px-4 text-right font-medium text-amber-800">{position.value}</td>
                  </tr>
                ))}
              </tbody>
            </table>
