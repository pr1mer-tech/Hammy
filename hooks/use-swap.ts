  ...
  const executeSwap = async (
    ...
    try {
      ...
      const parsedAmountOutMin = parseUnits(
        (Number.parseFloat(amountTo) * (1 - slippage / 100)).toFixed(tokenTo.decimals),
        tokenTo.decimals,
      )
      ...
      // Token -> Token
        ...
        const tx = await writeContract({
          ...
          args: [parsedAmountIn, parsedAmountOutMin, [tokenFrom.address, tokenTo.address], address, deadline],
        })
