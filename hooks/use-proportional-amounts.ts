...
export function useProportionalAmounts(
  ...
  useEffect(() => {
    const calculateProportionalAmount = () => {
      ...
      if (!amount || Number.parseFloat(amount) === 0 || !reserves || tokenA.address === tokenB.address) {
        setCalculatedAmount("");
        return;
      }
      ...
      try {
        ...
        const parsedAmount = parseUnits(amount, isTokenA ? tokenA.decimals : tokenB.decimals);
        
        let calculatedBigInt: bigint;
        if (isTokenA) {
          // amountB = amountA * reserveB / reserveA
          calculatedBigInt = (parsedAmount * reserveB) / reserveA;
          setCalculatedAmount(formatUnits(calculatedBigInt, tokenB.decimals));
        } else {
          // amountA = amountB * reserveA / reserveB
          calculatedBigInt = (parsedAmount * reserveA) / reserveB;
          setCalculatedAmount(formatUnits(calculatedBigInt, tokenA.decimals));
        }
      } catch (err) {
